...
def x_positioning(pressure_sens, t_W_B, r_W_B, t_B_C, r_B_C, t_C_P, r_C_P):
    ...
    if z_W_C == z_W_P:
        z_W_C = 10 # Avoding being divided by 0
    else:
...
def tilting_plus_slamyaw(tilting_angle,slamyaw):
    
    eul = np.array([tilting_angle, slamyaw])
    r_w2b = np.array(tf.transformations.quaternion_from_euler(eul[0], eul[1], eul[2]))
