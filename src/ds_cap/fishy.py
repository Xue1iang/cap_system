...
class TiltingEKF:
    ...
    def __init__(self):
        ...
        self.pub_cap_depth = rospy.Publisher('/cap_depth', Float64, queue_size=10)
        self.pub_cap_tag = rospy.Publisher('/cap_t_c2p', PoseStamped, queue_size=10)
        self.pub_cap_imu = rospy.Publisher('/cap_imu',Imu, queue_size=10)
        self.pub_cap_slam = rospy.Publisher('/cap_slam', PoseStamped,queue_size=10)        
    ...
    def callback_imu(self, imu):
        ...
        elif self.latest_values[1] is None:
        ...
        # else:
            ...
            self.t_w2m, self.depth, self.t_w2sb, self.r_w2b, self.t_c2p, self.r_c2p = xc_defs.ds_cap(self.depth, self.t_w2sb, self.r_w2b, self.t_b2c, self.r_b2c, self.t_c2p, self.r_c2p)
            ...
            captag = PoseStamped()
            captag.header.stamp = rospy.Time.now()
            captag.pose.position.x = self.t_c2p[0]
            captag.pose.position.y = self.t_c2p[1]
            captag.pose.position.z = self.t_c2p[2]
            captag.pose.orientation.w = self.r_c2p[3] 
            captag.pose.orientation.x = self.r_c2p[0]
            captag.pose.orientation.y = self.r_c2p[1]
            captag.pose.orientation.z = self.r_c2p[2]
            
            self.pub_cap_tag.publish(captag)

            capimu = Imu()
            capimu.header.stamp = rospy.Time.now()
            capimu.orientation.w = self.r_w2b[3]
            capimu.orientation.x = self.r_w2b[0]
            capimu.orientation.y = self.r_w2b[1]
            capimu.orientation.z = self.r_w2b[2]
            self.pub_cap_imu.publish(capimu)

            capslam = PoseStamped()
            capslam.header.stamp = rospy.Time.now()
            capslam.pose.position.x = self.t_w2sb[0]
            capslam.pose.position.y = self.t_w2sb[1]
            capslam.pose.position.z = self.t_w2sb[2]
            self.pub_cap_slam.publish(capslam)

            capdepth = Float64()
            capdepth.data = -self.depth
            self.pub_cap_depth.publish(capdepth)
