...
class TiltingEKF:
    ...
    def __init__(self):
        ...
        # Initialisation the conunter's reading list
        ...
        self.qualisys_subscriber = rospy.Subscriber('/qualisys/Mallard_pattern1/pose', PoseStamped, self.callback_qualisys)
    ...
    def callback_qualisys(self, qualisys):

        self.qualisys_t_w2b = np.array([qualisys.pose.position.x, qualisys.pose.position.y, qualisys.pose.position.z])
        self.qualisys_r_w2b = np.array([qualisys.pose.orientation.x, qualisys.pose.orientation.y, qualisys.pose.orientation.z, qualisys.pose.orientation.w])
    def callback_cam(self, pixed_centre):
        ...
        if pixed_centre.detections:
            ...
            if self.latest_values[1] is None:
            ...
            elif pixed_centre.detections[0].decision_margin > 19:
                # print(pixed_centre.detections[0].decision_margin)
                self.t_w2m = xc_defs.ds_cap(self.depth, self.t_w2sb, self.r_w2b, self.t_b2c, self.r_b2c, self.t_c2p, self.r_c2p)           
                # self.t_w2m = xc_defs.ds_cap(self.depth, self.qualisys_t_w2b, self.qualisys_r_w2b, self.t_b2c, self.r_b2c, self.t_c2p, self.r_c2p) 
                dsCAP = PoseStamped()
                ...
                # Rviz for qualisys (surface)
                # tf_qulisys = TransformBroadcaster()
                # tf_qulisys.sendTransform((self.qualisys_t_w2b[0], self.qualisys_t_w2b[1], self.qualisys_t_w2b[2]), (self.qualisys_r_w2b[0], self.qualisys_r_w2b[1], self.qualisys_r_w2b[2], self.qualisys_r_w2b[3]), rospy.Time.now(), 'Qualisys', 'World')
...
if __name__ == '__main__':
    ...
    rospy.init_node('ds_cap_camera_trigger')
    try:
        te = TiltingEKF()
